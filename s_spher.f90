...
2 format(a10)
 ...
 inquire(file='radius.xvg', exist=ex)

  if (ex) then

     call execute_command_line("mv radius.xvg radius_p.xvg")
     back = .true.

  end if

  open(4, file='radius.xvg', status='new', iostat=ierr)

  if(ierr /= 0) then

     write(*, *)
     write(*, *) 'Unable to open file radius.xvg'
     write(*, *)
     stop
  ...
  inquire(file='gyration.xvg', exist=ex)
  
  if (ex) then
     
     call execute_command_line("mv gyration.xvg gyration_p.xvg")
     back = .true.
     
  end if
  
  open(5, file='gyration.xvg', status='new', iostat=ierr)

  if(ierr /= 0) then

     write(*, *)
     write(*, *) 'Unable to open file gyration.xvg'
     write(*, *)
     stop
