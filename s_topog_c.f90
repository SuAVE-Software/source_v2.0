program suave_topog
  ...
  call startup(outer, bin, p_grid, coord, ind, ind2, rmsd, map, ind3, &
     ...
     n_grid, bin_out, fr_in, fr_end, n_skip, n_lipid, get, div, 's_topog   ', version)
...
! Leitura do arquivo .pdb
  ...
  do while (ierr >= 0)
     ...
     if ((frame>fr_in-1).and.(frame<fr_end+1).and.(eval_skip).and.(ierr>=0)) then
        ...
        call calc_topog(n_grid, aver, aver2, r_xpm1, grid, grid2)
